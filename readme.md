...
1. Export the following into your shell before running
    ...
    export PACT_BROKER_BASE_URL=<insert host here> # e.g. https://foo.pact.dius.com.au
    ```
...
1. Publish the contracts to PactFlow (ensure you're back in the projects' root directory)
    ```
    pact-broker publish --consumer-app-version 1.0.0 --broker-base-url $PACT_BROKER_BASE_URL --broker-token $PACT_BROKER_TOKEN ./pacts/consumer-provider.json --branch master
    ```
...
- [Running on PactFlow](#running-on-pactflow)
